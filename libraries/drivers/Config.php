...
/**
 * System_Config abstract driver to get and set
...
abstract class Config_Driver {
	 ...
	 * @var     string
	 */
	protected $cache_name = 'System_Config_Cache';
	...
	public function get($key, $slash = FALSE, $required = FALSE)
	{
		...
		$value = System::key_string($this->config, $key);
		...
		if (($required === TRUE) AND ($value === null))
			throw new System_Config_Exception('Value not found in config driver');
	...
	public function load_cache()
	{
		...
		$cached_config = System::cache($this->cache_name, $this->cache_lifetime);
	...
	public function save_cache()
	{
		...
		if ($this->get('core.internal_cache') !== FALSE AND $this->changed)
		{
			...
			// Save the cache
			return System::cache_save($this->cache_name, $data, $this->cache_lifetime);
		}

		return TRUE;
	}
} // End System_Config_Driver
