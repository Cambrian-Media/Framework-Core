 ...
 * System Exceptions
 *
 * $Id: System_Exception.php 4692 2009-12-04 15:59:44Z cbandy $
 *
 * @package    Core
 * @author     Kohana Team
...
class System_Exception extends Exception {
	...
	// Template file
	public static $template = 'system/error';
	...
	public function __construct($message, array $variables = NULL, $code = 0)
	{
		...
		if (is_array($variables)) {
			...
			{
				$message = str_replace($message, $var, $val);
			}
		}
	...
	/**
	 * Enable System exception handling.
	 *
	 * @uses    System_Exception::$template
	 * @return  void
	 */
	public static function enable()
	{
		if ( ! System_Exception::$enabled)
		{
			set_exception_handler(array('System_Exception', 'handle'));

			System_Exception::$enabled = TRUE;
		}
	}

	/**
	 * Disable System exception handling.
	 *
	 * @return  void
	 */
	public static function disable()
	{
		if (System_Exception::$enabled)
		{
			...
			System_Exception::$enabled = FALSE;
		}
	}
	...
	public static function text($e)
	{
		return sprintf('%s [ %s ]: %s ~ %s [ %d ]',
			get_class($e), $e->getCode(), strip_tags($e->getMessage()), System_Exception::debug_path($e->getFile()), $e->getLine());
	}
	...
	public static function handle(Exception $e)
	{
		try
		{
			...
			$error = System_Exception::text($e);

			// Add this exception to the log
			System_Log::add('error', $error);

			// Manually save logs after exceptions
			System_Log::save();
			
			if (System::config('core.display_errors') === FALSE)
			{
			...
			{
				$template = System_Exception::$template.$template;
				...
				if ($e instanceof ErrorException)
				{
					...
					$code = System::message('core.errors.'.$e->getSeverity());
			...
			if ($template = System::find_file('views', $template))
			{
				include $template;
			}
		}
	...
	{
		return System_Exception::$template;
	}
	...
	{
		return System_Exception::_dump($value, $length);
	}

	/**
	 * Helper for System_Exception::dump(), handles recursion in arrays and objects.
	 *
	 * @param   mixed    variable to dump
	 * @param   integer  maximum length of strings
			...
			if (($type = get_resource_type($var)) === 'stream' AND $meta = stream_get_meta_data($var))
			{
				...
				if (isset($meta['uri']))
				{
					...
					if (function_exists('stream_is_local'))
					{
						...
						{
							$file = System_Exception::debug_path($file);
						}
					}
		...
		elseif (is_array($var))
		{
			...
			elseif ($level < 5)
			{
				...
				foreach ($var as $key => & $val)
				{
					...
					$output[] = "$space$s$key => ".System_Exception::_dump($val, $length, $level + 1);
				}
				unset($var[$marker]);
		...
		elseif (is_object($var))
		{
			...
			elseif ($level < 5)
			{
				...
				foreach ($array as $key => & $val)
				{
					...
					$output[] = "$space$s$access $key => ".System_Exception::_dump($val, $length, $level + 1);
				}
				unset($objects[$hash]);
	 ...
	 *     echo System_Exception::debug_source(__FILE__, __LINE__);
	 *
	...
	public static function trace($trace = NULL)
	{
		...
		foreach ($trace as $step)
		{
			...
			if (isset($step['file']) AND isset($step['line']))
			{
				...
				$source = System_Exception::debug_source($step['file'], $step['line']);
			}
